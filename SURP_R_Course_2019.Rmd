...
These types of variables are numeric. However, R can also store text. These are in the form of characters. Surround characters in quote marks to have them be character and not variable names.
...
Characters variables can store strings, such as words and sentences. One string (everything between the quotation marks) is stored as a single unit.
```{r}
...
Finally, you can find out what variables you have loaded in your environment at anytime using `ls()`.
```{r}
ls()
```
...
You have already used some functions. Functions are always followed by `()` when you use them. 
```{r}
is(ls)
is(is)
```

When you use a function, often you are telling it to perform an operation on something else. That something else is called an argument, and passed to the function inside the parenthesis.
```{r}
var <- 1
is(var)
```
...
You can learn about a function by reading its documentation. You can access the documentation with `?`, for example, type `?seq` into your console.
