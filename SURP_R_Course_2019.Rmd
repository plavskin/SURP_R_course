...
In most programming languages, division is written using the slash ( __/__ ), and multiplication using the asterisk ( __*__ )
```{r}
...
Characters variables can store *strings*, such as words and sentences. In R, one string (everything between the quotation marks) is stored as a single unit.
```{r}
...
Strings can also hold numbers: just put a quote around them!
```{r}
num_string <- '4'
actual_num <- 4
...
We saw above that you can do basic arithmetic in R by just using mathematical symbols, e.g. __+__. But for more complicated things, programming languages use things called *functions*. A function is something that wraps a whole set of operations so that you can invoke those operations in a single command. Let's take a look at an example: there's a function, `sum()`, that can add numbers, just like __+__. Let's try it:
```{r}
variable1
variable2
...
Functions are always followed by `()` when you use them. When you use a function, often you are telling it to perform an operation on something else; in the example above, we're performing the `sum()` function on *variable1* and *variable2*. The 'something else' is called an argument, and is *passed* to the function inside the parenthesis.

However, some functions don't need to take any arguments. For example, you can find out what variables you have loaded in your *environment* at anytime using `ls()`. This will list all your variables, any custom functions you may have written, etc.
```{r}
ls()
```
...
Notice that when we used paste above, the function automatically put a space between our two strings. It's often really useful to combine strings with a different character, for example, a hyphen. Try to read the documentation for `paste()` to figure out how to do this.
```{r}
# Combine the research_program and year variables using just a hyphen between
# them instead of a space

```

Finally, another really useful function is `is()`. It tells you what type something is!
```{r}
is(ls)
is(is)
```
...
```{r}
# Use the is function to find the types for the var and char2 variables below
var <- 1
char2 <- 'b'
```
